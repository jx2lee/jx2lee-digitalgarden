---
{"dg-publish":true,"permalink":"/data_engineer/datahub/__troubleshooting/datahub-ts-unauthorized-when-ingestion/","dgPassFrontmatter":true}
---
...
# background
- datahub CLI 로 dbt 소스 ingest
- token 은 expired 하지 않은 설정으로 만들었지만 ingestion 중 401 unauthorized 오류 발생
```
...
## (X) 정상 토큰으로 요청 시 WARN 메시지가 발생하지 않는다?
...
## (O?) datahub-auth-secrets 시크릿을 재성성할 때마다 jwt key 가 변경된다?
- <u>(가정) 시크릿을 삭제한 내역이 있는데 이 영향으로 권한실패가 이루어지는지 확인하고 싶다.</u>
...
### datahub helm 템플릿을 살펴보던 중 datahub-auth-secrets 템플릿이 수상하다
```yml
...
{{- end -}}
```
- token_service_signing_key 값을 randAlphaNum 32 로 임의로 생성하고 있다.
