...
어느 정도의 확실성, 확률을 표현하는 새로운 방법과 새로운 형태의 베이즈 정리를 소개한다. 또한 evidence 의 중요한 개념과 강도를 정량화할 수 있는 방법에 대해서도 설명한다.

이 장의 두 번째로 addends(더하는 양과 그 분포를 계산하는 방법)에 대해 살펴본다. 합계, 차이, 곱셈 및 기타 연산의 분포를 계산하는 함수를 정의하는데, 베이지안 업데이트를 일부 사용한다.

# Odds
확률을 나타내는 한 가지 방법은 0과 1 사이의 숫자를 사용하는 것이지만, 이것이 유일한 방법은 아니다. 축구 경기나 경마에 베팅을 해본 적이 있다면 배당률이라는 또 다른 표현을 접해 보셨을 거다.

"배당률은 3 대 1입니다"와 같은 표현을 들어 보셨을 수도 있지만 그것이 무엇을 의미하는지 모를 수도 있다. odds in favor 은 이벤트가 발생할 확률과 발생하지 않을 확률의 비율을 말한다.
...
하지만 이 경우에 반대 확률이 9:1 이라고 표현하는 것이 일반적이다.
...
위 즈 정리를 odds 형식으로 표현할 수 있다.
$$\mathrm{odds}(A|D) = \mathrm{odds}(A)~\frac{P(D|A)}{P(D|B)}$$

bayes's rule 에 따르면 사후 확률은 이전 확률에 비율을 곱한 값이다. 예를 들어 쿠키 문제로 돌아가 보면 쿠키가 담긴 그릇이 두 개 있다고 가정해 해보자. 볼 1에는 바닐라 쿠키 30개와 초콜릿 쿠키 10개가 들어 있고, 볼 2에는 각각 20개가 들어 있다.

이제 두 그릇 중 하나를 무작위로 선택하고 보지 않고 무작위로 쿠키를 선택한다. 선택한 쿠키가 바닐라라면 그릇 1에서 나왔을 확률은 얼마인가?
...
Bayes's Rue 은 이러한 직관을 더욱 정확하게 만드는 방법을 제공한다.
...
- 좌변은 사후 확률과 사전 확률의 비율이다.
- 우변은 베이즈 계수(Bayes factor)라고도 하는 확률 비율(likelihood ratio) 이다.
...
- 위 문제에 적용해보자.
	...
	- 올리버가 현장에 혈액을 남기지 않았다면 두 개의 샘플을 설명할 수 있다. 모집단에서 무작위로 두 명을 선택하면 한 명은 'O형', 한 명은 'AB형'을 찾을 확률은 얼마나 될까?
...
그러나 조금 부족하다. 예를 들어, 사전 확률이 1(즉, 50% 확률)인 경우 사후 확률은 0.83이 되며 이는 45%의 확률에 해당한다.
...
첫 번째 예로 주사위 두 개를 굴려서 합산하면 합계의 분포는 어떻게 될까? 다음 함수를 사용하여 주사위의 가능한 결과를 나타내는 Pmf를 만들어 볼 수 있다.
...
- 두 개의 주사위를 굴려서 더하면 2부터 12까지 11가지 결과가 나온다.
- 하지만 그 확률이 같지는 않다.
- 합계의 분포를 계산하려면 가능한 결과를 열거(enumerate)해야 한다.
...
주사위 1개, 2개의 합, 3개의 합 분포는
- 단일 주사위의 분포는 1에서 6까지 균일하다.
- 두 주사위의 합계는 2에서 12 사이의 삼각형 분포를 갖는다.
- 주사위 3개 합은 3에서 18 사이의 종 모양 분포를 갖는다.

여담으로, 이 예는 합계의 분포가 적어도 일부 조건에서는 종 모양의 정규 분포에 수렴한다는 [중심 극한 정리](https://ko.wikipedia.org/wiki/%EC%A4%91%EC%8B%AC_%EA%B7%B9%ED%95%9C_%EC%A0%95%EB%A6%AC)를 보여준다.

# Gluten Sensitivity

- 2015년에 글루텐 민감성(셀리악병은 아님) 진단을 받은 사람들이 블라인드 챌린지에서 글루텐 밀가루와 비글루텐 밀가루를 구별할 수 있는지 테스트한 [논문](https://onlinelibrary.wiley.com/doi/full/10.1111/apt.13372)이 있다.
	- 35명의 피험자 중 12명은 글루텐 밀가루를 섭취하는 동안 증상이 재개되는 것을 기준으로 글루텐 밀가루를 정확하게 식별했.
	- 나머지 17명은 증상에 따라 글루텐이 함유되지 않은 밀가루를 잘못 식별했으며, 6명은 구분할 수 없었다.
- 저자는 "이중 맹검 글루텐 챌린지는 환자의 1/3에서만 증상 재발을 유도한다"고 결론지었다.

글루텐에 민감한 환자가 한 명도 없었다면 환자 중 일부는 우연히 글루텐 밀가루를 식별할 것으로 예상할 수 있기 때문에 이 결론은 이상하게 보일 수 있다. 그렇다면 이 데이터에 근거하여 글루텐에 민감한 피험자는 몇 명이고 추측하는 피험자는 몇 명일까?

- 베이즈 정리를 사용하여 이 질문에 답할 수 있지만, 먼저 몇 가지 모델링 결정(modeling descisions)이 필요하다.
	- 글루텐에 민감한 사람은 문제 조건에서 글루텐 밀가루를 정확하게 식별할 확률이 95%이고
	- 글루텐에 민감하지 않은 사람은 우연히 글루텐 밀가루를 식별할 확률이 40%이고, 다른 밀가루를 선택하거나 구분하지 못할 확률은 60%입니다.

이러한 특정 값은 임의적이지만 결과는 이러한 선택에 민감하지 않다.

이 문제는 두 단계로 해결한다. 먼저, 1) 피험자가 민감한지 알고 있다고 가정하고 데이터의 분포를 계산한다. 그런 다음 2) likelihood 을 사용하여 민감한 환자 수의 사후 분포를 계산한다.
...
- 민감한 피험자 대부분은 글루텐 가루를 정확하게 식별할 것으로 예상한다.
- 민감하지 않은 25명의 피험자 중 10명 정도는 우연히 글루텐 가루를 식별할 것으로 예상한다.
- 따라서 총 20개 정도의 정답을 맞출 것으로 예상한다.

민감한 피험자의 수만 주어진다면, 데이터의 분포를 계산할 수 있다.

## The Inverse Problem
데이터가 주어지면 민감한 피험자 수의 사후 분포를 계산한다.
...
- 각 값에 대해 올바른 식별의 총 개수 분포를 계산하고 그 결과를 판다스 데이터프레임에 열로 저장한다.
...
비교를 위해 또 다른 가능한 결과인 20개 정답에 대한 사후 결과도 계산해보자.

```python
1.7818649765887375
```

12, 20개 정답에 대한 사후 분포를 그리면 다음과 같다.

![](https://i.imgur.com/5RuEVNx.png)

- 정답이 12개였다면, 가장 가능성이 높은 결론은 글루텐에 민감한 피험자는 아무도 없다.
- 정답이 20개였다면, 가장 가능성이 높은 결론은 피험자 중 11~12명이 글루텐에 민감하다.
